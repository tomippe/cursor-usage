...
export type UsagePayload = {
  /** Display name from Stripe membership, e.g. Ultra / Pro. */
  planName: string | null;
  /** Legacy request-quota plans only; unused on Ultra spend plans (often 0/0). */
  ...
  /** Spending "Total" (`planUsage.totalPercentUsed`). */
  totalPercentUsed: number | null;
  /** Spending "First-party models" (`planUsage.autoPercentUsed`). */
  autoPercentUsed: number | null;
  /** Spending "API" (`planUsage.apiPercentUsed`). */
...
type AuthInfo = { userId: string; sessionToken: string; email: string | null; accessToken: string };

type PeriodUsageEnrichment = {
  totalPercentUsed: number | null;
  autoPercentUsed: number | null;
  apiPercentUsed: number | null;
  onDemandSpendDollars: number | null;
...
async function getCursorToken(): Promise<AuthInfo | null> {
  ...
  const info = { userId, sessionToken: `${userId}%3A%3A${jwt}`, email, accessToken: jwt };
  cachedAuth = { info, ts: Date.now() };
  return info;
}

async function fetchCurrentPeriodUsage(accessToken: string): Promise<PeriodUsageEnrichment | null> {
  try {
    ...
    const spendLimit = asRecord(data.spendLimitUsage);
    const totalPercentUsed = planUsage ? toNumber(planUsage.totalPercentUsed) : null;
    const autoPercentUsed = planUsage ? toNumber(planUsage.autoPercentUsed) : null;
    const apiPercentUsed = planUsage ? toNumber(planUsage.apiPercentUsed) : null;

    let onDemandLimitDollars: number | null = null;
    let onDemandSpendDollars: number | null = null;
    if (spendLimit) {
      const limitCents =
        toNumber(spendLimit.individualLimit) ?? toNumber(spendLimit.pooledLimit);
      const remainingCents =
        toNumber(spendLimit.individualRemaining) ?? toNumber(spendLimit.pooledRemaining);
      const usedCents =
        toNumber(spendLimit.individualUsed) ??
        toNumber(spendLimit.pooledUsed) ??
        (limitCents !== null && remainingCents !== null
          ? Math.max(0, limitCents - remainingCents)
          : null);
      if (limitCents !== null && limitCents > 0) {
        onDemandLimitDollars = limitCents / 100;
      }
      if (usedCents !== null) {
        onDemandSpendDollars = usedCents / 100;
      }
    }

    const billingCycleEndMs = toNumber(data.billingCycleEnd);

    log(
      `Period usage: total=${totalPercentUsed}% auto=${autoPercentUsed}% api=${apiPercentUsed}% onDemand=$${onDemandSpendDollars ?? "?"}/$${onDemandLimitDollars ?? "?"}`,
    );

    return {
      totalPercentUsed: clampPercent(totalPercentUsed),
      autoPercentUsed: clampPercent(autoPercentUsed),
      apiPercentUsed: clampPercent(apiPercentUsed),
      onDemandSpendDollars,
      onDemandLimitDollars,
      billingCycleEndMs,
    };
  } catch (error) {
    const msg = error instanceof Error ? error.message : String(error);
    log(`GetCurrentPeriodUsage error: ${msg}`);
    return null;
  }
}

function applyPeriodUsage(
  result: UsagePayload,
  period: PeriodUsageEnrichment | null,
  onDemandEnabled: boolean,
): UsagePayload {
  if (!period) return result;

  if (period.totalPercentUsed !== null) {
    result.totalPercentUsed = period.totalPercentUsed;
  }
  if (period.autoPercentUsed !== null) {
    result.autoPercentUsed = period.autoPercentUsed;
  }
  if (period.apiPercentUsed !== null) {
    result.apiPercentUsed = period.apiPercentUsed;
  }

  if (period.billingCycleEndMs !== null && Number.isFinite(period.billingCycleEndMs)) {
    result.resetsAt = new Date(period.billingCycleEndMs).toISOString();
  }

  if (onDemandEnabled && period.onDemandLimitDollars !== null && period.onDemandLimitDollars > 0) {
    result.onDemand = {
      state: "limited",
      spendDollars: period.onDemandSpendDollars ?? result.onDemand.spendDollars,
      limitDollars: period.onDemandLimitDollars,
    };
  }
...
/** Normalize stripe membershipType → status-bar label (Ultra, Pro, …). */
export function formatPlanName(raw: unknown): string | null {
...
async function ensureSetup(
  ...
  const planName = formatPlanName(
    stripeRecord?.individualMembershipType ??
      stripeRecord?.membershipType ??
      stripeRecord?.teamMembershipType,
  );
  ...
  log(
    `Setup cached: plan=${cachedSetup.planName}, team=${cachedSetup.isTeamMember}, teamId=${cachedSetup.teamId}, maxReq=${cachedSetup.maxRequestUsage}, onDemandEnabled=${cachedSetup.onDemandEnabled}`,
  );
  return cachedSetup;
}
...
async function fetchTeamUsage(
  ...
  applyPeriodUsage(result, period, setup.onDemandEnabled);
  ...
  log(
    `Result: ${result.totalPercentUsed !== null ? `${result.totalPercentUsed.toFixed(1)}%` : `${result.includedRequests.used}/${result.includedRequests.limit} reqs`}, $${result.onDemand.spendDollars.toFixed(2)}/${spendLimitLabel}`,
  );
  return result;
}
...
async function fetchSoloUsage(
  ...
  log(
    `Result: ${result.totalPercentUsed !== null ? `${result.totalPercentUsed.toFixed(1)}%` : `${result.includedRequests.used}/${result.includedRequests.limit} reqs`}`,
  );
  return result;
}
